...
- **待办也在其中** — 添加、完成、编辑和清除只保存在本机的轻量任务
- **一个紧凑界面** — 在 Agents、Spotify 与待办之间切换，不必来回切换 app 窗口
- **更精致的展开界面** — 更清晰的层级、原生感控件、更舒适的对比度与颜色，以及展开状态的布局修复
- **个性化 Island** — 隐藏可选标签页，设置动态/静态/隐藏音乐状态，并按显示器配置会话列表

## Open Island 是什么？

Open Island 驻留在 Mac 的**刘海区域**（或顶部栏），为你的 AI coding agents 提供实时控制面板——会话状态、权限审批、一键跳回正确的终端。本 fork 还把 Spotify 播放和本地待办列表放进同一个展开界面，让 agents、音乐和下一步行动始终只需看一眼。
...
| 展开标签页 | Agents、Spotify 控件和本地待办；Spotify 与待办可分别隐藏 |
| 个性化 | 按显示器配置收起状态和会话列表，并可设置动态/静态/隐藏音乐状态 |
| 设置 | Hook 安装/卸载、用量仪表盘、通用、个性化、显示、声音、快捷键、实验室、关于 |
...
- **设置** — Hook 安装/卸载、Codex/Claude 用量仪表盘、通用、个性化、显示、声音、快捷键、实验室（高级）、关于
