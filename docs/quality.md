...
The renderer preserves the real notch occlusion from the supplied take, establishes Agents briefly, then centers the Spotify reveal, expanded controls, and compact playback while work continues. It adds a punch-in, pull-back, and moving SF-system captions before exporting a silent 1920×1080, 60 fps H.264 master under `output/launch-video/`. It does not add a demo-data label to the product surface or the video.
