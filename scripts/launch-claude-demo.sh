...
if [[ "$(uname -s)" != "Darwin" ]]; then
    echo "The Open Island launch demo runs only on macOS." >&2
    exit 1
fi
...
bundle_dir="$demo_root/Open Island Launch Demo.app"
...
echo "Launching the local Codex + Claude demo. No live hooks or session history will be used."
...
if [[ -z "$demo_pid" ]]; then
    echo "Open Island launch demo failed to launch." >&2
    exit 1
fi
