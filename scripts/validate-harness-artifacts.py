...
def main() -> None:
    ...
    if scenario == "closed":
    ...
    elif scenario in ("calendarAgenda", "calendarMonth", "calendarYear", "calendarConnect", "calendarEmpty", "calendarDenied"):
        if notch_status != "opened" or report.get("selectedTab") != "calendar":
            fail("Calendar scenario did not open the Calendar tab")
        require_frame_between(overlay_frame, width=(440, 510), height=(350, 430), context="Calendar overlay frame")
        assert_contains_any(button_labels, ["Calendar"], "Calendar tab")
        assert_contains_any(button_labels, ["Today"], "Calendar date navigation")
        for view in ("Day", "Month", "Year"):
            assert_contains_any(button_labels, [view], "Calendar view selection")
        ...
        elif scenario == "calendarMonth":
            assert_contains_any(button_labels, ["Previous month", "Next month"], "Calendar month navigation")
            assert_contains_any(text_values | labels, ["Choose a day to see its events"], "Calendar month hint")
        elif scenario == "calendarYear":
            for month in ("January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"):
                assert_contains_any(button_labels, [month], "Calendar year months")
